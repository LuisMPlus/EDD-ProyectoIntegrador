...
public class BinaryTree<ELEMENT extends Comparable<ELEMENT>> {
    ...
    protected int LeafCount(BTNode<ELEMENT> root) {
        if (root != null) {
            //Condicion de hoja, que no tiene descendientes
    ...
    protected int InternalCount(BTNode<ELEMENT> root) {
        if (root != null) {
            //Condicion de nodo interno. Que no es una hoja, osea esta entre la raiz(inclusive) y las hojas
            if ( (root.left == null) && (root.right == null) ) {
                return 0;
            } else {
